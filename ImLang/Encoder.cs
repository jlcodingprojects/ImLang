...
namespace ImLang
{
    public static class Encoder
    {
		...
		*/
        ...
        public static List<byte> IntToBytes(int num)
        {
        ...
        public static List<byte> Concatentate(params IEnumerable<byte>[] bytes)
        {
            List<byte> list = new List<byte>();

            for (int i = 0; i < bytes.Length; i++)
            {
                var newBits = new List<byte>(bytes[i].Select(x => x));
                list.AddRange(newBits);
            }
        ...
        public static List<byte> CreateSection(byte section, List<byte> bytes)
        {
            bytes = Wrap(bytes);
            bytes.Insert(0, section);
            return bytes;
        }
    }
